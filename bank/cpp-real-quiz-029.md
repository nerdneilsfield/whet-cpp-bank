...
正确答案是 C。
[dcl.array]/1：C++ 数组的大小必须是 constant expression；n 在编译期未知，所以 int arr[n] 不是标准 C++。
VLA（Variable-Length Array）是 C99 的核心特性，C++ 标准从未接受（C++14 曾尝试以 "arrays of runtime bound, ARB" 引入，被否决）；GCC、Clang 默认提供 VLA 作为扩展，所以你写这段代码用 g++ a.cpp 可能编译过、运行正常输出 4n。
但用 g++ -std=c++17 -pedantic 会给警告，用 MSVC 直接编译错误： 正确的 C++ 写法：std::vector<int> arr(n);。
