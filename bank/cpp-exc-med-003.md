...
正确答案是 B。`noexcept` 是不抛异常承诺；异常若逃出该函数会调用 `std::terminate`。选项 B 的表述“调用 ｀std::terminate()｀，程序异常终止，**不进行栈展开**到 ｀main｀ 的 ｀catch｀”正好符合该规则。A、C、D 的问题通常在于把相近概念混同、忽略默认行为，或把运行期现象误认为编译期/标准规定。
