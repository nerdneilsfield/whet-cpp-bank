...
作答应覆盖：指出读取未初始化的对象（除 unsigned char/byte 外）是 UB；解释为什么不是简单地读到"随机值"（编译器可能假定不会发生）；给出常见出错场景（栈上的 int 未初始化、new 出的非类对象）。未定义行为题的关键是区分“标准无保证”和“某次运行看起来正常”；一旦触发 UB，编译器可基于其不会发生来优化。 简答题不只要求给结论，还要说明为什么这些规则成立以及错误用法会造成什么后果。常见失分点是只背术语，没有联系代码场景或边界条件。
