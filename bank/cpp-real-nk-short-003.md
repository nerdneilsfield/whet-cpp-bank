...
本题评分重点是 vtable 编译期生成并通常放在只读段，vptr 存在于多态对象中并在构造/析构过程中由编译器写入。单继承通常只有一个 vptr，覆盖函数替换虚表槽位；多继承可能有多个 vptr，并用 thunk 调整 this 指针。构造和析构期间虚调用不会分发到尚未构造或已经析构的派生部分，而是按当前构造层级调用。常见误区是在构造函数中期待调用派生类 override，这会得到与普通运行期多态不同的结果。
