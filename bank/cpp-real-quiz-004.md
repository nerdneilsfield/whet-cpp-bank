...
正确答案是 B。
C++11 之前，负整数除法与取模的商如何处理（向零截断还是向下取整）是实现定义行为；C++11 起，[expr.mul]/4 明确规定：商向零截断（truncation toward zero），即 -7 / 2 = -3（不是 -4）。
取模满足恒等式 (a/b)b + a%b == a：-3 2 + (-1) == -7，所以 -7 % 2 = -1；易混淆答案 A (-4 -1) 对应的是被 Python 等语言采用的"floor division" 策略（向负无穷取整）。
