...
本题评分重点是右值引用、std::move、移动构造/移动赋值和五法则之间的关系。移动语义把临时对象或将亡值中的资源“接管”过来，源对象被置为可析构的有效状态，从而避免深拷贝。Rule of Five 强调管理资源的类若自定义析构、拷贝构造或拷贝赋值，通常也要考虑移动构造和移动赋值。常见误区是认为 std::move 会移动对象；它只是类型转换，真正移动由匹配到的移动操作完成。
