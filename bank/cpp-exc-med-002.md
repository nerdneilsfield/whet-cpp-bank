...
正确答案是 C。`throw` 表达式用于抛出异常；在 `catch` 中单独写 `throw;` 表示重新抛出当前异常。选项 C 的表述“｀catch｀ 捕获到，打印消息后异常被**自动重新抛出**，｀Outer｀ 构造失败”正好符合该规则。A、B、D 的问题通常在于把相近概念混同、忽略默认行为，或把运行期现象误认为编译期/标准规定。
