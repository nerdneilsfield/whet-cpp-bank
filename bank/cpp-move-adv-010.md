...
正确答案是 B，`std::forward<T>` 根据 `T` 是否为引用类型决定返回左值引用还是右值引用。它的目标是恢复原始实参的值类别，而不是像 `std::move` 那样无条件转成右值。若把两者混用，可能意外移动调用者传入的左值。 这类题的关键是区分语法形式、对象生命周期和所有权语义：语法看似相近时，应先判断谁拥有资源、表达式值类别是什么，以及标准是否保证该行为；不要根据实现习惯或表面写法推断答案。
