...
正确答案是 A。
*逗号运算符 expr1, expr2 依次求值 expr1、expr2，返回最右侧表达式的结果（[expr.comma]/1）。
第一行：(i++, i + 10, i) i++ → i 变成 1 i + 10 → 结果是 11，丢弃 i → 结果是 1（当前 i 值） 输出 1。
第二行：(i += 10, i * 2, i) i += 10 → i 变成 11 i * 2 → 结果是 22，丢弃 i → 结果是 11 输出 11。
