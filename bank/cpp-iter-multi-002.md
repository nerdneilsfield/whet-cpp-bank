...
本题正确答案是 A, B, D。本题考查迭代器类别、失效规则或迭代移动语义，关键是看容器操作后原迭代器是否仍指向有效元素。A 项正确：“vector::pushback 触发容量增长时，所有原迭代器失效。”符合题干要求；B 项正确：“unorderedmap 在 rehash（如 insert 触发负载因子超限）时，所有迭代器失效。”符合题干要求；C 项错误：“list::splice 操作总是使所有迭代器失效。”与题干要求或 C++ 规则不符；D 项正确：“删除关联容器中的某个元素，只会让指向该元素的迭代器失效，其它迭代器仍有效。”符合题干要求。多选题常见误区是漏选同时成立的规则，或把只在特定上下文成立的说法当成普遍规则。
