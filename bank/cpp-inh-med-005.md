...
正确答案是 D，对应“using Base::f;”。本题考查继承中的访问控制、构造析构顺序、虚函数或多继承规则，关键是区分静态类型限制和运行期动态绑定。其他选项容易混淆相关概念：A 项“virtual void f(double) = 0;”不满足题干要求；B 项“void f(double) override;”不满足题干要求；C 项“Base::f(3.14);”不满足题干要求。常见误区是只凭关键字判断，而没有结合完整声明、表达式求值结果或 C++ 语义限制。
