...
本题评分重点是说明 vptr、vtable 和动态分发路径：多态对象通常含有隐藏 vptr，指向类的虚表，虚调用时通过 vptr 找到槽位中的函数地址再跳转。单继承下派生类虚表复用基类布局，覆盖函数替换原槽位，新虚函数追加；多继承下可能有多个 vptr，并通过 thunk 调整 this 指针。示意图只要能表达对象到虚表再到函数入口即可。常见误区是以为虚表存在于每个对象中；通常对象保存 vptr，虚表由同类对象共享。
