...
A、C、D 正确：模板模板参数的匹配在 C++17 前更严格，默认模板实参不一定能让 `std::vector` 匹配单参数模板形参。C++17 起模板模板参数中的 `typename` 与 `class` 等价，但实参仍必须是类模板或别名模板。常见误区是把函数模板、变量模板也当成可传入的 template template argument。
